/*
 * @LastEditTime: 2021-06-14 21:41:03
...
const maxValue = d3.max(chartData.map(d => d.values[year]))
const highlight = (e, d) => {
...
const title = g => g.append("title").text(d => `${years[year]} - ${d.territory}\n${d3.format("$,.2f")(d.values[year])}`)
...
const data = d3.csvParse(await FileAttachment("sales4yrs.csv").text(), d3.autoType)
...
const drawRadialBars = function (x, maxValue, maxRadius, margin, deg, minRadius, bar, chartData, color, arc, innerRadius, year, title, highlight, restore, seq, axisArc, parts) {
  return (
    (g, tspace) => {
...
const drawStartLines = function (chartData, color, bar, innerRadius, numOfBars, minRadius, margin, start, title, highlight, restore, d3, year, triangle, seq, parts) {
  return (
    g => {
