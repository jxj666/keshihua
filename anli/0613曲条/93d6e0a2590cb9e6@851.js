...
md`# Fancy Radial Bar Chart
Surprisingly, it does look like a jumbo swirl lollipop 🍭😋`
)});
  ...
  main.variable(observer("chart")).define("chart", ["d3","width","height","drawStartLines","drawRadialBars"], function*(d3,width,height,drawStartLines,drawRadialBars)
{
  ...
  main.variable(observer("drawStartLines")).define("drawStartLines", ["chartData","color","bar","innerRadius","numOfBars","minRadius","margin","start","title","highlight","restore","d3","year","triangle","seq","parts"], function(chartData,color,bar,innerRadius,numOfBars,minRadius,margin,start,title,highlight,restore,d3,year,triangle,seq,parts){return(
g => {  
...
() => {
  ...
  main.variable(observer("title")).define("title", ["years","year","d3"], function(years,year,d3){return(
...
d3.scaleLinear()
  ...
  main.variable(observer("color")).define("color", ["d3","chartData"], function(d3,chartData){return(
...
a => a * 180 / Math.PI
  ...
  main.variable(observer("arc")).define("arc", ["d3","innerRadius","outerRadius","x","year"], function(d3,innerRadius,outerRadius,x,year){return(
(d, i) => d3.arc()
...
d3.csvParse(await FileAttachment("sales4yrs.csv").text(), d3.autoType)
  ...
  main.variable(observer("chartData")).define("chartData", ["data"], function(data){return(
data.map(d => {  
  return {
  ...
  main.variable(observer("maxValue")).define("maxValue", ["d3","chartData","year"], function(d3,chartData,year){return(
d3.max(chartData.map(d => d.values[year]))
)});
  main.variable(observer("year")).define("year", ["options"], function(options){return(
options.year
)});
